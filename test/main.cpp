...
BOOST_DATA_TEST_CASE(route_prefix, all_test_backends(), kind, backend)
{
    ...
    {
        MOCK_EXPECT(second_ctx).once().with([](const hyperoute::route_context &ctx) {
            return ctx.matched_path == "/static_2/a" && ctx.remaining_path == "/file/path"
                && ctx.params.find("id")->second == "a";
        });
...
BOOST_DATA_TEST_CASE(route_verb, all_test_backends(), kind, backend)
{
...

    {
        ...
        MOCK_EXPECT(first_ctx)
            .once()
            .with([](const hyperoute::route_context &ctx) { return ctx.matched_path == "/test"; })
            .in(s);

        MOCK_EXPECT(second_ctx)
            .once()
            .with([](const hyperoute::route_context &ctx) { return ctx.matched_path == "/"; })
            .in(s);
...
BOOST_DATA_TEST_CASE(add_route_failure_same_param_name, all_test_backends(), kind, backend)
{
    ...
    const auto router = builder.build(ec);
    BOOST_TEST(
        (ec == hyperoute::error::duplicate_parameter),
        "check ec == hyperoute::error::duplicate_parameter has failed [ec == " << ec.message() << "]"
    );
...
BOOST_DATA_TEST_CASE(add_route_failure_regex_syntax, regex_test_backends(), kind, backend)
{
    ...
    const auto router = builder.build(ec);
    BOOST_TEST(
        (ec == hyperoute::error::regex_syntax),
        "check ec == hyperoute::error::regex_syntax has failed [ec == " << ec.message() << "]"
    );
    BOOST_TEST(router.has_value() == false);
}
